...
path <- file.path(directory,"1_demux/true_hits")	 		# Where the fastq demuxd files are located
...
seqtab2 <- seqtab[,nchar(colnames(seqtab)) %in% 103:113] # lengths set as 103-113 bp
...
head(track)
write.csv(track, "12S_track_fwd_orient_track.csv")
save.image("dada2.RData")

uniquesToFasta(seqtab.nochim, "ASVs_12S_1.fasta", ids=colnames(seqtab.nochim))
save.image("dada2.RData")
