...
DCC_table_combo <- read.csv("Rev_abundance_table.csv", header = T, sep=",")

DCC_table_combo <- DCC_table_combo %>% mutate(seq_id=row_number())
blast <- read.csv("ASV_rev_blast_top_hit.csv", header = FALSE, sep="\t")
...
write.csv(My_table_combo,"Rev_combined_results.csv")