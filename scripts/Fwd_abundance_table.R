...
### LIB1 ###
sd <- read.csv("track_fwd_orient.csv", header = T, sep=",")
sd <- column_to_rownames(sd, "X")
sd <- phyloseq(sample_data(sd))
otu1 <- readRDS("seqtab_nochim.rds")
...
rdp_output="RDP_classified_fwd.txt" #CHANGE ME to the path to your RDP output
...
write.csv(DCC_table_combo,"Fwd_abundance_table.csv")
