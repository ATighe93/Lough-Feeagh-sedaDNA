# dada2 pipeline Rscript for Sonic #

library("dada2")

directory = getwd()
setwd(file.path(directory,"rev_dada"))			# Where you want the output files to save
path <- file.path(directory,"2_demux/true_hits") # Where the fastq demuxd files are located

## This first part can be run in terminal real time to observe the quality plots before proceeding ##

fnFs <- sort(list.files(path, pattern="_1.fastq", full.names = TRUE)) 	   # list forward files 
fnRs <- sort(list.files(path,pattern="_2.fastq", full.names = TRUE)) 	   # list reverse files
...
out <- filterAndTrim(fnFs, filtFs, fnRs, filtRs, truncLen=c(108,108), minLen=70, maxN=0, maxEE=c(2,2), truncQ=2, rm.phix=TRUE, compress=TRUE, multithread=TRUE)
...
seqtab2 <- seqtab[,nchar(colnames(seqtab)) %in% 103:113] # lengths set as 106 bp
...
head(track)
write.csv(track, "12S_track_rev_orient.csv")
save.image("dada2.RData")
...
uniquesToFasta(seqtab.nochim2, "ASVs_12S_rev.fasta", ids=colnames(seqtab.nochim2))
save.image("dada2.RData")
saveRDS(seqtab.nochim2, "seqtab_nochim2.rds")
